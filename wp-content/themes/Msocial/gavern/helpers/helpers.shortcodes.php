...
if(get_option($tpl->name . '_shortcodes3_state', 'Y') == 'Y') {  
	 ...
	 *
	 **/
	if(!function_exists('gavern_tss_contact_form')) {
		// Usage:
		// [contactform name="yes" email="yes" title="yes" captcha="yes" mailto="blank@gavick.com"]
		function gavern_tss_contact_form($atts, $content) {   
			global $tpl;
			// extract the attributes
			extract(
				shortcode_atts(
			...
			$output .= '<form action="'.get_current_page_url().'" method="post">';
			
			if($name == 'yes') {
				$output .= '<p><input type="text" placeholder="'.__('Name', GKTPLNAME).'" name="gkcontact-name"></p>';
			}
			
			if($email == 'yes') {
				$output .= '<p><input type="email" placeholder="'.__('e-mail', GKTPLNAME).'" name="gkcontact-email"></p>';
			}
			
			if($title == 'yes') {
				$output .= '<p><input type="text" placeholder="'.__('Title', GKTPLNAME).'" name="gkcontact-title"></p>';
			}
			
			$output .= '<p><textarea name="gkcontact-textarea" placeholder="'.__('Your message', GKTPLNAME).'"></textarea></p>';
			...
			if(
				...
				$output .= '<p><script type="text/javascript">var RecaptchaOptions = { theme : \'clean\' };</script>'.recaptcha_get_html($publickey).'</p>';
			}
			
			$output .= '<p><input type="submit" value="'.__('Send Message', GKTPLNAME).'" class="bigbutton"></p>';
			
			if(isset($mailto) && trim($mailto) != '') {
				$encoded_email = '';
				
				for ($i = 0; $i < strlen($mailto); $i++) {
					$encoded_email .= "&#" . ord($mailto[$i]) . ';';
				}
			
				$output .= '<input type="hidden" name="gkcontact-mailto" value="'.$encoded_email.'" />';
			}
		...
		// add the shortcode to system
		add_shortcode('contactform', 'gavern_tss_contact_form');
	}
}  
