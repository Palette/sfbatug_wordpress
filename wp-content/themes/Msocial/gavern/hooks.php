...
function gavernwp_body_attributes_hook() {
 	...
 	if(is_home()) {
 		$classes = '';
 		if($tpl->browser->get("browser") == 'ie8') {
 			$classes .= ' ie8mode';
 		} 
 		
 		if(isset($_COOKIE[$tpl->name . '_solid']) && $_COOKIE[$tpl->name . '_solid'] == 1 || get_option($tpl->name . '_pattern_bg') != 0) {
 			$classes .= ' solidBg';
 		}
 		
 		if(isset($_COOKIE[$tpl->name . '_pattern']) && is_integer($_COOKIE[$tpl->name . '_pattern']) || get_option($tpl->name . '_pattern_bg') != 0) {
 			if(isset($_COOKIE[$tpl->name . '_pattern'])) { 
 				$classes .= ' pattern' . $_COOKIE[$tpl->name . '_pattern'];
 			} else {
 				$classes .= ' pattern' . get_option($tpl->name . '_pattern_bg');
 			}
 		}
