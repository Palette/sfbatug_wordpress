...
class GK_NSP_Widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		$amount_of_posts = ($config['article_pages'] * $config['article_cols'] * $config['article_rows']) + ($config['links_pages'] * $config['links_cols'] * $config['links_rows']);
		// resutls array
		$results = array();
		// data source
		...
		// wrap articles
		if($amount_of_articles <= count($results) && $config['links_pos'] != 'bottom') {
			echo '<div class="gk-nsp-arts" style="width: '.(100 - $config['links_width']).'%;" data-pos="'.$config['links_pos'].'">';
		} else {
		...
		if($amount_of_articles <= count($results)) {
			...
			$links_pages_amount = 0;
			$start = $amount_of_articles;
			// in order to avoid division by 0
			if($this->wdgt_config['links_cols'] > 0) {
				$links_pages_amount = ceil($amount_of_links / ($this->wdgt_config['links_rows'] * $this->wdgt_config['links_cols']));
			}
			// generate the links
			if($amount_of_links > 0) {
				if($config['links_pos'] == 'bottom') {
					echo '<div class="gk-nsp-links">';
					echo '<div>';
				} else {
					echo '<div class="gk-nsp-links" style="width: '. $config['links_width'] .'%;">';
					echo '<div style="margin: '.$config['links_margin'].'">';
				}
				
				echo '<div class="gk-nsp-links-scroll gk-nsp-pages-'.$links_pages_amount.'">';
				
				for($i = 0; $i < $links_pages_amount; $i++) {
					echo '<div class="gk-nsp-list gk-nsp-cols-'.$config['links_pages'].'">';
					for($k = 0; $k < $this->wdgt_config['links_cols']; $k++) {
						echo '<ul class="gk-nsp-list gk-nsp-cols-'.$config['links_cols'].'">';
		...
		//
		if($this->wdgt_config['links_rows'] > 0 && $links_pages_amount > 1) {
			echo '<div class="gk-nsp-links-nav">';
			
			if($config['links_pagination'] != 'arrows') {
				echo '<ul class="gk-nsp-pagination">';
				
				for($i = 1; $i <= $links_pages_amount; $i++) {
					echo '<li>' . $i . '</li>';
				}
	...
	function form($instance) {
		...
		$links_cols = isset($instance['links_cols']) ? esc_attr($instance['links_cols']) : '0';
		$links_rows = isset($instance['links_rows']) ? esc_attr($instance['links_rows']) : '0';
		$links_pos = isset($instance['links_pos']) ? esc_attr($instance['links_pos']) : 'bottom';
		$links_width = isset($instance['links_width']) ? esc_attr($instance['links_width']) : '50';
		$links_margin = isset($instance['links_margin']) ? esc_attr($instance['links_margin']) : '0';
	...
	?>	
		<div class="gk-nsp-col">
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'links_cols' ) ); ?>"><?php _e( 'columns:', GKTPLNAME ); ?></label>
				<input id="<?php echo esc_attr( $this->get_field_id( 'links_cols' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'links_cols' ) ); ?>" type="text" value="<?php echo esc_attr( $links_cols ); ?>" class="short" />
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'links_pos' ) ); ?>"><?php _e( 'position:', GKTPLNAME ); ?></label>
				
				<select id="<?php echo esc_attr( $this->get_field_id('links_pos')); ?>" name="<?php echo esc_attr( $this->get_field_name('links_pos')); ?>">
					<option value="bottom"<?php selected($links_pos, 'bottom'); ?>>
						<?php _e('Bottom', GKTPLNAME); ?>
					</option>
					<option value="right"<?php selected($links_pos, 'right'); ?>>
						<?php _e('Right', GKTPLNAME); ?>
					</option>
					<option value="left"<?php selected($links_pos, 'left'); ?>>
						<?php _e('Left', GKTPLNAME); ?>
					</option>
				</select>

				<label for="<?php echo esc_attr( $this->get_field_id( 'links_width' ) ); ?>"><?php _e( 'width:', GKTPLNAME ); ?></label>
				<input id="<?php echo esc_attr( $this->get_field_id( 'links_width' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'links_width' ) ); ?>" type="text" value="<?php echo esc_attr( $links_width ); ?>" class="short" />%
			</p>
			
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'links_margin' ) ); ?>"><?php _e( 'Links block margin:', GKTPLNAME ); ?></label>
				<input id="<?php echo esc_attr( $this->get_field_id( 'links_margin' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'links_margin' ) ); ?>" type="text" value="<?php echo esc_attr( $links_margin ); ?>" class="medium" />
			</p>
	 ...
	 **/
	 ...
	 function generate_art_title($i) {
	 	...
	 	$part_one = explode(' ', $art_title_short);
	 	$part_one = $part_one[0];
	 	$part_two = (count(explode(' ', $art_title_short)) > 1) ? substr($art_title_short, strpos($art_title_short,' ')) : '';
	 	
	 	$header_span =  '<span>'.$part_one.'</span>'.$part_two;
	 	
	 	$output = '<h3 class="gk-nsp-header"><a href="'.$art_url.'" title="'.str_replace('"','', $art_title).'">'.$header_span.'</a></h3>';
	 		...
	 		if(!is_wp_error($img_editor)) {
		 		...
		 		if(is_string($new_path)) {
		 			...
		 			if($this->wdgt_config['article_image_pos'] == 'left' && $this->wdgt_config['article_image_order'] == 1) {
		 				return apply_filters('gk_nsp_art_image', '<div class="gk-nsp-image-wrap"><a href="'.$art_url.'" class="gk-image-link" '.$style.'><img src="'.$new_path.'" alt="" class="gk-nsp-image" height="'.$this->wdgt_config['article_image_h'].'" width="'.$this->wdgt_config['article_image_w'].'" /></a></div>');
		 			} else {
	 ...
	 function generate_art_readmore($i) {
	 	...
	 	$output = '<a href="'.$art_url.'" class="btn readon" title="'.__('Read more', GKTPLNAME).'">'.__('Read more', GKTPLNAME).'</a>';
